...
public class Graph{
	...
	Set<DirectedEdge>[] rever_Graph_Map;//反向链接表
	...
	Graph(int max_vertex){
		...
		rever_Graph_Map=(Set<DirectedEdge>[])new Set[max_vertex];
		for(int i=0;i<max_vertex;i++)
			rever_Graph_Map[i]=new HashSet<DirectedEdge>();
	}
	public void addEdge(DirectedEdge e){
		Graph_Map[e.SourceID].add(e);
		DirectedEdge rever_e=new DirectedEdge(e.DestinationID,e.SourceID,e.E_Name,e.Cost);
		rever_Graph_Map[e.DestinationID].add(rever_e);
		E++;
