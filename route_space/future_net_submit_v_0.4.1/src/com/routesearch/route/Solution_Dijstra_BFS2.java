...
class Info2{
	...
	static Comparator<Info2> OrderIsdn =  new Comparator<Info2>(){  
...
class Prior_Path2{
	...
	Prior_Path2(List<DirectedEdge> path,int inc_count,int Cost){
		this.path=path;
		this.inc_count=inc_count;
		this.Cost=Cost;
		this.disable_inc=new HashSet<>();
	}
	static Comparator<Prior_Path2> OrderIsdn =  new Comparator<Prior_Path2>(){  
        public int compare(Prior_Path2 o1, Prior_Path2 o2) {  
            // TODO Auto-generated method stub  
...
public class Solution_Dijstra_BFS2 extends Thread{
	...
	void Dijstra_BFS_SP(PriorityQueue<Prior_Path2> path){
		boolean delete_flag=false;
		while(!path.isEmpty()){
			Prior_Path2 prior_path=path.poll();
			...
			Info2[] info=new Info2[graph.V];
			for(int i=0;i<info.length;i++)
				info[i]=new Info2(i);
			int startID=prior_path.path.get(0).DestinationID;
			info[startID].distTo=0;
			PriorityQueue<Info2> queue=new PriorityQueue<>(graph.V,Info2.OrderIsdn);
			...
			if(inc_len==0){
				...
				List<Prior_Path2> son_path=new ArrayList<>();
				while(!queue.isEmpty()){
					Info2 temp=queue.poll();
					if(this.inc_node[temp.id]&&temp.id!=startID&&Cost+info[temp.id].distTo<Min_Cost){
						...
						newpath.addAll(prior_path.path);
						if(pre_disable_id!=-1){
							for(Prior_Path2 p:son_path)
								if(p.path.get(0).DestinationID==pre_disable_id)
									p.disable_inc.add(temp.id);						
						}
						son_path.add(new Prior_Path2(newpath,inc_count,Cost+info[temp.id].distTo));
						if(inc_len==0)break;
					}
		        	for(/*DirectedEdge e:graph.Graph_Map[temp.id]*/DirectedEdge e:graph.rever_Graph_Map[temp.id]){
		        		if(visit[e.DestinationID]==false&&info[e.DestinationID].distTo>temp.distTo+e.Cost
	...
	public void FindPath(int StartID, int EndID, Set<Integer> IncVertexs,
		...
		PriorityQueue<Prior_Path2> path=new PriorityQueue<>(100,Prior_Path2.OrderIsdn);
    	this.inc_node=new boolean[graph.V];
    	for(int id:IncVertexs)
    		this.inc_node[id]=true;
    	List<DirectedEdge> newpath=new ArrayList<>();
    	DirectedEdge e=new DirectedEdge(this.StartID,this.StartID,0,0);
    	newpath.add(e);
    	path.add(new Prior_Path2(newpath,0,0));
		Dijstra_BFS_SP(path);
		*/
	}
