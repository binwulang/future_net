...
public class Solution_Dijstra_DFS extends Thread{
	 ...
	 */
	/*Dijstra_DFS_SP1*/
	void Dijstra_DFS_SP1(int StartID, int EndID,List<DirectedEdge> path,int Cost){
		...
		while(!queue.isEmpty()){
			...
			if(IncVertexs.contains(temp.id)
					&&Cost+info[temp.id].distTo<Min_Cost){
				...
				Dijstra_DFS_SP1(temp.id,EndID,path,Cost);
				Cost-=info[temp.id].distTo;
	...
	public void run() {
		...
		Dijstra_DFS_SP1(StartID,EndID,path,Cost);	
	}
